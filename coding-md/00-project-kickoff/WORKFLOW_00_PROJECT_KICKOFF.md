...
- テンプレート: `04_Templates/wordpress-base` 側の WordPress 設計テンプレートを参照（静的ベースには WP 設計書一式は置かない）
- 案件別設計書: 案件リポジトリの `coding-md/07-wordpress-design/WORDPRESS_DESIGN.md` など

#### 実装優先順位（全 WP 案件共通）

1. 静的コーディング（partials 共通化）← 本スターターの主対象
2. WordPress テーマ化
3. CPT 実装
...
- レイアウト境界・確定 clamp は `.cursor/rules/site-layout-tokens.mdc` に案件ごとに記録する
- WordPress 化は静的完成後。WP ルール・設計テンプレートは `wordpress-base` 側を使う
